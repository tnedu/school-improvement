  ...
  pmap(
    ...
    .f = ~ ..1 %>%
      ...
      scale_y_continuous(
        ...
        limits = c(0, pmin(100, 1.25 * max(..1[[..3]], na.rm = T))) # ..5 # c(0, 100)
      ) +
  ...
  pmap(
    ...
    .f = ~ ..1 %>%
      ...
      scale_y_continuous(
        ...
        limits = c(0, pmin(100, 1.25 * max(..1[[quo_name(..2)]], na.rm = T))) # c(0, 100)
      ) +
      scale_fill_manual(
  ...
  pmap(
    ...
    .f = ~ ..1 %>%
      ...
      mutate(
        ...
        # )
        fill_var = case_when(
          column_name == "school_rank_exited_2020_ytd" ~ -1 * cell_value,
...
table_school <-
  pmap(
    .l = list(
      ...
      fill_colors_low = c(rep("darkseagreen", 2), "deeppink3"),
      fill_colors_high = c(rep("tomato", 2), "purple"),
      limits = list(
        c(1, nrow(schools_csi)),
        c(1, nrow(schools_csi)),
        c(-1 * nrow(schools_csi), nrow(schools_csi))
      ),
      midpoints = c(rep(median(1:nrow(schools_csi)), 2), 0),
    ...
    .f = ~ ..1 %>%
      ...
      geom_text(
        ...
        aes(label = cell_label), hjust = 1, nudge_y = 0.2, size = 5
      ) +
      geom_text(
        data = ..1 %>% filter(str_detect(column_name, "Rank")),
        aes(label = cell_label), size = 5
      ) +
      coord_flip() +
      scale_y_discrete(
      ...
      labs(
        ...
        caption = str_wrap(
          str_c("School ranks show where schools stand among all 92 CSI schools ",
                "(1 corresponds to the lowest percent and is shaded ", ..7[[1]], ", 92 corresponds to the ",
                "highest and is shaded ", ..7[[2]], "). The 2018-19 Year-to-Date column ",
                "shows each school's ", ..2, " at this time last year."),
          width = 100
        )
      )
  ) %>%
  set_names(nm = c("absence", "discipline", "mobility"))

```

```{r graphs_teacher, include=FALSE, eval=FALSE}
