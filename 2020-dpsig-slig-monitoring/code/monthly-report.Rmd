...
# directory_onedrive <- "C:/Users/CA20397/TN Dept of Education/Data Use Team - Documents/"
# directory_project <- "projects-master/school-improvement/2020-dpsig-slig-monitoring/"
...
if(params$district == 0) {
  ...
  #         by = "district"
  #       ) %>%
  #       filter(!str_detect(measure_name, "AMO")) %>%
  #       mutate(measure_name = str_c("ytd_", school_year)) %>%
  #       select(-school_year, -last_date_of_month, -starts_with("n_")) %>%
  #       spread(measure_name, !!.y) %>%
  #       mutate(pct_diff = round(100 * (ytd_2020 - ytd_2019) / ytd_2019, 1)) %>%
  #       mutate_at(vars(district), factor) %>%
  #       mutate_at(
  #         "calendar_month",
  #         funs(factor(., levels = c(8:12, 1:6), labels = month_levels))
  ...
  graph_line_state <-
    pmap(
      .l = list(
        data_frames = temp, # append(temp, temp_2),
        y_variables_quo = quos(
          pct_students_chr_absent,
          pct_students_disciplined,
          pct_students_entered,
        ...
        limits = list(
          ...
          c(0, 100),
          c(0, 100)
          # c(min(temp_2[[1]]$pct_diff) - 5, max(temp_2[[1]]$pct_diff) + 5),
          # c(min(temp_2[[2]]$pct_diff) - 5, max(temp_2[[2]]$pct_diff) + 5),
          # c(min(temp_2[[3]]$pct_diff) - 5, max(temp_2[[3]]$pct_diff) + 5),
          # c(min(temp_2[[4]]$pct_diff) - 5, max(temp_2[[4]]$pct_diff) + 5)
        )
        # colors = list(
        #   c("skyblue", "red"),
        #   c("skyblue", "orange"),
...
# After some exploration, a state-level version of this graph did not seem
...
graph_bar_school <-
  pmap(
    .l = list(
      ...
      # fill_variables = append(
      #   rep(quos(in_district), 4),
      #   rep(quos(district_name), 4)
      # ),
      # fill_names = c(
      #   rep("School in your district?", 4),
      #   rep("", 4)
      # ),
...
for(i in 1:length(district_list)) {
  ...
  temp <-
    pmap(
      ...
      .f = ~ ..1 %>%
        filter(district %in% d) %>% # == params$district
        left_join(schools_csi, by = c("district", "school")) %>%
...
if(params$district == 0) {
  ...
  #             ) +
  #             labs(
  #               title = str_c("Percent of Students ", .y, " in ", dn)
  #             )
  #         )
  #     }
  #   ) %>%
    ...
    map2(
      ...
      ~ temp %>%
        ...
        theme(
          ...
          # strip.text = element_text(vjust = 5)
        ) +
        labs(
  ...
  # print(table_school_3_year$`190`$absence)
  ...
  table_school_turnover <-
    map2(
      ...
      ~ turnover_school %>%
        ...
        labs(
          title = str_c("Year-to-Year Teacher Exits in ", .y) # str_c("Percent of Students ", .y, " in ", dn)
        )
    ) %>%
    set_names(nm = as.character(districts_csi$district))
...
# ![]("output/archive/graph-size-test.png")
...
if(params$district > 0) {
  ...
  #   print(table_school[[d]]$absence)
  # }
  # walk(
  #   .x = as.character(district_list),
  #   .f = ~ print(table_school[[.x]]$absence)
  # )
